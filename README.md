...
### Chart-independent tetrahedron masses (`--ps-vertex-mass`)

By default each stream tetrahedron carries mass ρ̄·V_lag (its unperturbed Lagrangian volume times the mean density) — exact **only when the Lagrangian positions are the unperturbed lattice**. For TNG the `--lagrangianInput` (`combined_ics.hdf5`) holds the *evolved* z=127 IC snapshot, whose configuration already carries the density contrast δ_ic = D(z_ic)/D(z)·δ(z): tetrahedra in future-overdense regions are pre-compressed, and ρ̄·V_lag then under-weights them, filtering **every density mode by 1 − D(z_ic)/D(z)** (measured −16.5% at z=20, −3% at z=2 for z_ic=127; velocities are unaffected). `--ps-vertex-mass` instead gives each tetrahedron `m_tet = Σ_v (particle mass)/(number of finite tetrahedra incident to vertex v)`, i.e. each particle's mass split equally among its incident tetrahedra, so the mass follows the particles and the bias vanishes at any z_ic. Total mass is conserved exactly (the degree pass counts only deposit-surviving tetrahedra, so dropped-tet vertex shares renormalize onto the kept ones); the cost is small-scale degree noise from the local Delaunay connectivity. Applies to the CPU, GPU and `--ps-exact-deposit` deposits; the `--sample-points` per-stream density estimators and the `--ps-halo-release` classification are unchanged. **`run_ps_dtfe.sh` sets this ON by default** (`PS_VERTEX_MASS=1`) because every TNG run needs it; set `PS_VERTEX_MASS=0` only for true-lattice inputs. Validated by `tests/ps_vertex_mass_check.sh` (mass conservation, degree-pass idempotency, and the 1−α contrast-transfer measurement on a perturbed-IC pancake).

### Volume-weighted velocity moments (`--ps-volume-weighted`)

The default velocity/gradient/divergence/shear/vorticity moments are **mass-weighted** (momentum-like, Σ ρ_s v_s / Σ ρ_s). `--ps-volume-weighted` makes them **Eulerian-volume-weighted** instead — the standard-DTFE `_a` convention that linear theory's −aHf·δ divergence refers to. The choice is **per field**: the velocity family switches, but the **dispersion deliberately stays mass-weighted** (σ_ij is an f-weighted moment of the collisionless Boltzmann equation by definition) and carries its own mean and normalizer, so `.velDisp`/`.velDispTensor` come out **bit-identical to a default run**. Density and `.streams` are also unchanged. The upshot: this one configuration is the literature-standard estimator for every field at once (which is why `run_ps_pipeline.sh` uses `PS_VOLUME_WEIGHTED=1`). Works with the CPU and GPU deposits; rejected in combination with `--ps-linear-deposit`. Validated by `tests/ps_volume_weighted_check.sh`.
...
* **`--pts-vel-grad`** also writes the **velocity gradient** at each point: `.pts_velGrad` (float64 × 9 per point, row-major `[d·3+j]` = ∂v_j/∂x_d — the same layout as the grid `.velGrad`). It is the density-weighted mean over the point's streams of each stream's constant linear-velocity-profile gradient (the pointwise analogue of the grid velocity-gradient deposit). **Divergence, shear and vorticity follow algebraically** from this tensor, so no separate files are needed — `dtfelib.PointPlane` derives them (see the point-evaluation figure pipeline below). Note that being a spatial derivative of a piecewise-linear field, it is **constant within each tetrahedron** and discontinuous across faces, so per-point derivative maps look faceted at full resolution unless smoothed (the figure pipeline writes a smoothed companion automatically).
...
The model behind the tuner: large runs are bounded by the per-partition CGAL triangulation (~0.65 KB per padded vertex; the Lagrangian padding multiplies particle counts by ~4x) plus the global field grids. PS-DTFE partitions **Lagrangian** space with `--partition X Y Z` and caps concurrent partition pipelines with `--max-concurrent N`; peak RSS ≈ globals + N × (per-partition triangulation + sub-grids). Manual reference points (what auto should land near): a 512³ grid with the default field set fits TNG50-4 (2e7 particles) at `--partition 2 2 2 --max-concurrent 2` in ≲30 GB, and TNG300-3 (2.4e8 particles) at `--partition 5 5 5 --max-concurrent 2` in ~40 GB. The `run_ps_dtfe.sh` runlog reports the measured peak RSS after every run, closing the loop on the prediction. With `--sample-points`, the tuner also accounts for the point-evaluation records (24 B/point + mean-streams × 168 B), which are global and no partition can shrink — a 8192² image plane at z=0 is ~35 GB on top of the grids.

The **irreducible** term is the full-resolution field grids themselves: `--partition` cannot reduce it (partitions sum into one shared grid), so the full production field set caps in-RAM runs around ~600³. When even a single split is predicted over budget the tuner refuses honestly, naming the GB and the largest grid that would fit, rather than picking a doomed split.

### Out-of-core grids (`--scratch-dir <local dir>`)

To run past the RAM ceiling (e.g. 1024³ with the full field set — ~146 GB of accumulators vs a 55 GB budget), `--scratch-dir` backs the full-grid accumulators (any allocation ≥ 1 GB; `DTFE_SCRATCH_MIN_GB` overrides) with **mmap'ed, immediately-unlinked files** in the given directory instead of RAM. Results are bit-identical to an in-RAM run, peak RSS stays bounded, and the scratch files free themselves on any exit (including a crash) because they are unlinked at creation. The auto-tuner drops the grid term from its RAM model when armed and checks the scratch volume's **free space** instead. The directory **must be local and non-synced** — iCloud/network paths and nonexistent dirs are rejected at option parsing (a mmap'ed 146 GB scratch on iCloud would try to sync-upload). `run_ps_dtfe.sh` exposes it via the `SCRATCH_DIR` env var. Measured acceptance: 1024³ full production fields on a 64 GB machine → exit 0, peak RSS 49.8 GB, ~146 GB of accumulators on disk, outputs bit-identical to the in-RAM path (`tests/ps_scratch_check.sh`, run locally — not in CI, as it uses `statvfs`/`/private/tmp`).
...
SIMS="TNG100-3-Dark" ./run_ps_pipeline.sh               # compute-only pipeline: grids + point-eval image planes for every snapshot
```

`run_ps_dtfe.sh` env toggles map to the flags above: `PS_VERTEX_MASS` (default 1), `PS_VOLUME_WEIGHTED`, `PS_EXACT`, `PS_METAL` (or `-m`), `SCRATCH_DIR`, `SAMPLE_POINTS` (+ `PTS_VEL_GRAD`), `OUTPUT_PREFIX` (write an alternate `<prefix>.*` grid set side by side), and `FIELDS`. [run_ps_pipeline.sh](run_ps_pipeline.sh) is **compute-only** (no plotting): for every snapshot of every simulation it regenerates the production grids **and** point-evaluates a full-box high-resolution image plane on the *same* triangulation (so the slice costs only the evaluation on top of the grid run). It is resumable (skips snapshots whose `.pts_*` are newer than their inputs; `FORCE=1` overrides), generates one plane per simulation via `make_image_plane.py`, and keeps the `.pts_*` files (they are the figure inputs). Render the figures separately with `plot_pointeval.py` (below).
...
* `dtfelib.io.FieldSet` — uniform loader for DTFE and PS-DTFE outputs (units, naming, averaged/raw); `velocity_single_stream()` returns the velocity with NaN outside single-stream (stream count ≠ 1) regions — PS-DTFE only. Reads memory-safely: fields within a RAM budget load eagerly, larger ones return a read-only memmap, with `load_slice()` / `iter_slabs()` / `field_stats()` for per-plane and streaming access (a 1024³ velGrad plots at MB-level RSS). `prefix=` reads an alternate `OUTPUT_PREFIX` grid set.
* `dtfelib.cli` — shared CLI (`--sim`, `--snap`, `--method`, `--smooth`, `--prefix`, …) used by every script in `python/plot/`.
* `python/plot/*.py` — one figure family per script (density/web maps, eigenvalues, correlations, void shapes, …).
* **Point-evaluation figure pipeline** — grid-free, publication-resolution field maps from a `--sample-points` run (the continuous field sampled at pixel positions, resolution limited only by the tessellation, not by any grid):
  * `python/tools/make_image_plane.py` — writes the pixel-centre sample points of an image plane (`--sim`/`--snap`, `--nu` pixels, `--axis` or `--axes xyz` for all three orthogonal views, optional slab `--planes`/`--supersample`) as the raw `.bin` `--sample-points` input plus a `.json` geometry sidecar.
  * `dtfelib.io.PointPlane` — reads the `.pts_*` outputs + sidecar; exposes `density`, `streams`, `velocity`, `dispersion` and the **derived** `speed`, `dispTrace`/`dispMag`, and `velDiv`/`velShear`/`velVort` (from `.pts_velGrad`); applies the same unit scaling as `FieldSet`.
  * `python/plot/plot_pointeval.py` — renders one annotated figure per field (axes, colorbar, thesis colormaps, native resolution) into `figures/fields/<sim>/snapNNN/<plane>/`. The faceted gradient fields also get a Gaussian-smoothed companion (`..._smooth<σ>.png`, σ=10 px default; `--smooth-derivatives`). `plot/plot_pointeval_panels.py` is the standalone three-panel (density / sheet / N_streams) variant with a pixel-exact `--image` export.
  * `python/tools/compare_deposits.py` — A/B a sampled vs `--ps-exact-deposit` grid set (mass conservation, empty-cell fraction, the low-density PDF tail, cell-to-cell scatter) to decide whether the exact deposit is worth its cost for a given analysis.
