...
#ifndef PADDING_TEST_HEADER
#define PADDING_TEST_HEADER
...
inline void insertDummyTestParticles(DT &dt,
                            User_options &userOptions)
{
    ...
    {
#if NO_DIM==2
        vh = dt.insert( *it, vh->face() );  // reuse previous cell as the location hint (2D)
#else
        vh = dt.insert( *it, vh );          // reuse previous vertex as the location hint (3D)
#endif
...
inline void showCellsContainingDummyPoints(vector<size_t> *dummyGridCells,
                                    User_options &userOptions,
                                    string outputName,
                                    string quantityName,
