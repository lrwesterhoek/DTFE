...
/* Reader for HDF5 Gadget snapshots that store positions as 32-bit integers (Cristian's variant). */
#ifdef HDF5
#include <H5Cpp.h>
#include <math.h>
using namespace H5;
...
// Reads Gadget data from a single HDF5 file (one of possibly several per snapshot). Variant where
// positions are stored as 32-bit integers (IntegerCoordinates).
                                  ...
                                  size_t *numberParticlesRead)
{
    ...
    if ( userOptions.readParticleData[0] )
    {
        ...
        for(int type=0; type<6; type++)
        {
            ...
            // positions are stored as 32-bit integers spanning [0, 2^32) over the box
...
// Reads the data from single or multiple HDF5 files (Cristian integer-coordinate variant).
